...
export function normalizeKiwifyEvent(parsed: KiwifyWebhookPayload): NormalizedGatewayEvent {
  ...
  return {
    ...
    buyerPhoneHash,
    buyerDocumentHash,
    // Plain IP/UA pra Meta CAPI EMQ (1.4.9). Schema parser declara ip +
    // IPCheckout (variant) + user_agent. `?? undefined` converte null Zod.
    clientIpAddress: customer.ip ?? customer.IPCheckout ?? undefined,
    clientUserAgent: customer.user_agent ?? undefined,
    affiliateEmailHash,
