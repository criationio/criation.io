  ...
  run: async (payload: { correlationId?: string } = {}) => {
    ...
    return withCorrelation(cid, async () => {
      ...
      if (connections.length === 0) {
        logger.info('meta-token-refresh: nenhum token precisando refresh', { correlationId: cid })
...
export const metaTokenRefreshCron = schedules.task({
  ...
  run: async () => {
    ...
    return withCorrelation(cid, async () => {
      logger.info('meta-token-refresh-cron disparou', {
        correlationId: cid,
        ts: new Date().toISOString(),
      })
      const handle = await metaTokenRefreshTask.trigger({ correlationId: cid })
