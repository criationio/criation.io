...
export const googleTokenRefreshTask = task({
  ...
  run: async () => {
    ...
    if (connections.length === 0) {
      logger.info('google-token-refresh: nenhuma conexao Google ativa')
      return { processed: 0, refreshed: 0, expired: 0, failed: 0, durationMs: Date.now() - start }
    }
...
export const googleTokenRefreshCron = schedules.task({
  ...
  maxDuration: 300,
  run: async () => {
    logger.info('google-token-refresh-cron disparou', { ts: new Date().toISOString() })
    const handle = await googleTokenRefreshTask.trigger()
