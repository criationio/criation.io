...
**Próxima sessão:** 1.4.9.B — Google EC fanout + OAuth Google antecipado. Cadeia restante: **1.4.9.B** (Google EC) → **1.4.9.C** (cleanup `click_id_store` legacy) → **1.4.9.5** (shadow validation E2E em prod real, gate antes de 1.6). 1.4.9 (Meta CAPI) fechada 2026-05-12.
...
**Bloqueios pré-cliente real:** GATE pré-prod [1.4.8 checklist](./docs/checklists/PRE-PROD-VALIDATION-1.4.8.md) (UTM Stitcher) **+** TD-104 LGPD erasure path **absorvidos pela sessão 1.4.9.5** — executados com dado real de prod sobre o conjunto 1.4.8 + 1.4.9, não em isolado.

| Fase                     | Status          | Início     | Fim        | Notas                                                                                                                                           |
| ------------------------ | --------------- | ---------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| Fase 0 — Pré-dev         | ✅ Concluído    | 2026-04    | 2026-05-07 | 4 sessões + correção pós-0.5                                                                                                                    |
| Fase 1 — Core Value      | 🟡 Em andamento | 2026-05-07 | —          | 1.1-1.4.8 + sub-1.4 + 1.4.A + 1.4.B + 1.4.9 (Meta CAPI) fechadas; 1.4.9.B (Google) → 1.4.9.C (cleanup) → 1.4.9.5 (shadow E2E) gate antes de 1.6 |
...
- [ ] **1.4.9.B** — Google EC fanout + OAuth Google antecipado (~6-8h) ⚠️ **crítico** — [ADR-014](./docs/adr/ADR-014-criation-as-cdp.md) + [audit Google 2026-05](./docs/audits/GOOGLE_API_2026-05.md). **Por que antecipa parte de 2.10:** ADR-014 promete fanout multi-platform desde Fase 1; sem token Google não dá pra enviar pra Google EC. **Pré-requisito:** **spike de 2-3h Data Manager API** (audit Google §4) — `UserDataService` deprecando 1-abr-2026 pra tokens novos; precisa confirmar `ConversionUploadService` (`UploadClickConversions`) é o endpoint correto pra 1.4.9.B vs migrar pra Data Manager API. **Schema deltas:** `google_connections` 14 colunas P1 (`grantedScopes`, `managerCustomerId`, `loginCustomerIdHeader`, `developerTokenTier`, `adsApiVersion default 'v24'`, `oauthClientVerificationStatus`, `googleUserId/Email/Name`, `lastTokenRefreshAt`, `tokenRefreshFailures`, `refreshTokenInvalidatedAt`, `partnerAgent`, `testAccountFlag`), `customer_id`/`customer_name` ficam `@deprecated`; **tabela nova `google_ads_accounts`** (1:N de `google_connections` — multi-customer Agency); **tabela nova `google_conversion_action_mappings`** (workspace + ad_account + internal_event_name → conversion_action_resource_name — **sem isso, fanout Google falha 100%**); `tracking_events` ganha `gad_source` + `srsltid` (audit §3); `capi_events` ganha 9 colunas Google-specific (`google_customer_id`, `google_conversion_action_resource_name`, `google_click_id_used`, `google_click_id_type`, `google_user_identifiers_count`, `google_consent_ad_user_data`, `google_consent_ad_personalization`, `google_order_id`, `google_adjustment_type`). **OAuth Google flow:** Server Action `connectGoogle`, callback `/api/auth/google/callback` (state + PKCE), scopes `https://www.googleapis.com/auth/adwords` only (não pedir `analytics.*` aqui), endpoint `customers:listAccessibleCustomers` + per customer `SELECT customer_client...FROM customer_client` (lista MCC + managed accounts) + `SELECT conversion_action...FROM conversion_action WHERE status='ENABLED'`. Persistir tudo em `google_ads_accounts` + popular `conversion_actions jsonb`. **OAuth verification submit em paralelo** (não no fim — 2-6 semanas timeline). **Developer Token tier:** aplicar Basic imediatamente, Standard quando ≥5 workspaces ativos. **Service:** `capi/google.adapter.ts` — builder payload `ConversionUploadService.UploadClickConversions` (conversion_action resource name + conversion_date_time + conversion_value + currency_code + order_id + gclid OR wbraid OR gbraid fallback ladder + user_identifiers hashed (`hashed_email`, `hashed_phone_number`, `address_info`) + consent {ad_user_data, ad_personalization}). Header `login-customer-id` obrigatório quando MCC. Rate limit: 2000 conversions/request máx + 15k ops/dia GLOBAIS no developer token Basic (Trigger.dev priority queue por workspace). **Trigger.dev:** `fanout-google-ads.ts` task (batch até 2000 conversions/request, atualiza `tracking_events.fanout_google_status`/`_sent_at`/`_error`). **UI:** wizard Conversion Action mapping em `/configuracoes/google/conversoes` (CRUD `google_conversion_action_mappings` — cada `event_name` Criation → conversion_action Google), multi-customer picker pós-OAuth, warning se Basic tier em workspace prod. **Token refresh:** estender `token-refresh.service.ts` pra `google_connections` (job 1x/24h — refresh tokens Google não expiram por tempo mas detectam `invalid_grant` cedo). **PII hashing:** reusar `capi/hashing.ts` da 1.4.9 (Google quer trim+lowercase, Meta também strip diacritics no nome — handler centralizado por provider). **Cross-platform dedup:** mesmo `event_id` enviado pra Meta + Google + pixel cliente (Meta dedupa, Google dedupa por `order_id`).

- [ ] **1.4.9.C** — Cleanup tabela legacy `click_id_store` (~30min, PR single) — tabela vestigial da arquitetura v0.5 (observer mode); ADR-014 (CDP) moveu os dados equivalentes pra `tracking_events.fbclid/gclid/ttclid/msclkid/ctwa_clid/wbraid/gbraid/fbp/fbc` + `tracking_visitors.first/lastClickId`. **Confirmação:** grep no `src/` mostra **zero writers, zero readers** — só declaração no schema + re-export do tipo. **Migration single (não 3-PR zero-downtime — regra 16 só aplica com dual-write ativo):** `0013_drop_click_id_store.sql` — drop 4 indexes (`workspace_id`, `expires_at`, `fbclid` parcial, `gclid` parcial) + drop table. **Code:** remover declaração `clickIdStore` em `src/lib/db/schema/tracking.ts`, remover `ClickId` type export em `src/lib/db/schema/index.ts`. **Motivo do timing (entre 1.4.9.B e 1.4.9.5):** schema fica estável **antes** da validação shadow. Dead schema é vetor de drift — dev novo lê tabela com tipos+indexes definidos, pensa "isso é canônico" e escreve nele em vez de `tracking_events`. PR pequeno e isolado, review trivial.

- [ ] **1.4.9.5** — Shadow validation end-to-end em prod real (~8-10h trabalho ativo + 5-7d wait calendar) ⚠️ **gate arquitetural antes de 1.6 — não pule**. **Motivo:** subsistema de tracking (1.4.A captura + 1.4.B matcheia + 1.4.8 atribui + 1.4.9 envia Meta + 1.4.9.B envia Google + 1.4.9.C schema limpo) está **feature-complete** neste ponto. Até aqui validado contra fixtures sintéticas (testes Vitest + smoke E2E manuais por sessão). Bugs reais (iOS Safari ITP, conflito com pixel/GTM pré-existente do cliente, encoding pt-BR em payload Hotmart prod, UTM `{{ad.name}}` literal em campanhas Meta com 5+ anos de naming caótico, EMQ baixo por hash format errado, dedup com pixel do cliente quebrado por event_id divergente, LDU flag não respeitando Consent state, Google Conversion Action mapping errado, MCC `login-customer-id` header faltando) só aparecem com dado real. **Última janela barata antes de 1.6/1.7 exporem o dado ao cliente** — matcher com falso positivo faz cliente ver venda atribuída à campanha errada no dashboard e perder confiança em 5min. Bug interno é barato; bug visível ao cliente é caro. Esta sessão valida **o conjunto 1.4.8 (UTM Stitcher) + 1.4.9 (Meta CAPI) + 1.4.9.B (Google EC) em prod**, não em isolado. **Setup (sequencial):** (1) deploy staging com domínio próprio + HTTPS estável (Vercel `gru1`) servindo `criation-tracking.js` via CDN; (2) snippet instalado em 1 landing real (site da AGC ou cliente amigo alpha); (3) 1 conexão Hotmart **prod** ativa (opcional Kiwify/Eduzz); (4) Meta ads **e Google Ads** ativos R$200-500 cada com convenção UTM correta apontando pra landing instrumentada; (5) consent banner mínimo pra Consent Mode v2 read funcionar; (6) Meta Events Manager + Test Events configurados pra ler EMQ shadow; (7) Google Ads conversion actions com Enhanced Conversions habilitado em modo teste. **Camada 1 — captura (1.4.A + fix fbp/fbc da 1.4.9):** script carrega sem conflito com pixel/GTM, ITP Safari iOS não quebra cookie `_cio_vid`, Consent Mode v2 read funciona, eventos chegam no volume esperado, partição mensal `tracking_events` aguenta, rate limit 600/min não estoura, **`_fbp` gerado quando ausente** (validação do fix da 1.4.9), **`_fbc` setado quando fbclid presente**. **Camada 2 — matching (1.4.B):** visitor↔buyer dispara em venda real, reverse matching via `identify` funciona em fluxo lead magnet→compra, sem falsos positivos cross-workspace (RLS aguenta tráfego real), `matched_buyer_email_hash` retroativo populado nos `tracking_events` históricos. **Camada 3 — atribuição (1.4.8):** stitcher matcheia campanhas Meta+Google reais (não fixture), TD-083 (`{{ad.name}}` literal) resolvido automaticamente, aggregates em `campaigns` incrementam atomic sem race, [PRE-PROD-VALIDATION-1.4.8.md](./docs/checklists/PRE-PROD-VALIDATION-1.4.8.md) executado com dado real. **Camada 4 — fanout Meta (1.4.9):** **EMQ baseline ≥7 no Events Manager** após 7d, event_id dedup com pixel cliente confirmado (sem double-count em Events Manager → Diagnostics), hash format conforme spec Meta (SHA-256 lowercase trim), `action_source` correto por evento type, LDU respeita Consent state, fbp/fbc presentes em ≥80% dos eventos (validação do fix do script), `capi_events` log preserva payload + response code. **Camada 5 — fanout Google (1.4.9.B):** **Google EC match rate ≥60%** no conversions report, header `login-customer-id` correto em MCC, `conversion_action_resource_name` mapeia corretamente por evento, gclid OR wbraid OR gbraid fallback funciona, consent field `ad_user_data`/`ad_personalization` propagado de `tracking_events.consent_state`, decisão Data Manager API validada (ou refactor flaggado se a API canônica mudou). **Compliance:** TD-104 LGPD erasure path simulado com titular request real (visitor + buyer + capi_events apagados sem residual). **Entregáveis:** `docs/smoke/1.4.9.5-shadow-validation-e2e.md` documentando cenários executados por camada, bugs encontrados (severidade P0/P1/P2), fixes aplicados, **EMQ baseline Meta + Google EC match rate** (números reais, não suposição), novos TDs abertos. **Definição de pronto:** zero P0 nas 5 camadas; EMQ Meta ≥7 (com 7d de dado); Google EC match rate ≥60%; GATE 1.4.8 + TD-104 verdes com dado prod; decisão escrita go/no-go pra 1.6 com lista do que mudou (ou não) na especificação do dashboard baseado no que o dado real revelou.
- [ ] **1.5** — Onboarding wizard 7 passos + alocação de 50 créditos signup_bonus (~4h)

#### Semana 2 — Dashboard & Campanhas

- [ ] **1.6** — Dashboard funil 8 etapas + topbar com saldo de créditos (~5h) ⚠️ **pré-req: 1.4.9.5 verde** — primeira tela que expõe dado de tracking ao cliente; bug em matcher/stitcher vira "venda atribuída à campanha errada" e cliente perde confiança em 5min
