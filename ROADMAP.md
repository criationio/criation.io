...
**Próxima sessão:** 1.4.9 — CAPI fanout sender (Meta + Google EC). Logo após: **1.4.9.5 — Shadow validation end-to-end em prod real** (valida 1.4.8 stitcher + 1.4.9 sender contra dado real, antes de 1.6 expor tracking ao cliente).
...
| Fase 1 — Core Value      | 🟡 Em andamento | 2026-05-07 | —          | 1.1-1.4.8 + sub-1.4 + 1.4.A + 1.4.B fechadas; 1.4.9 próx, depois 1.4.9.5 (shadow prod E2E) gate antes de 1.6 |
...
**Sessões:** 1.1 → 1.15 (21 sessões, incluindo intermediárias 1.4.5-1.4.9, 1.4.9.5, 1.7.5, 1.14.5)
...
- [ ] **1.4.9** — CAPI fanout sender (Meta + Google Enhanced Conversions) (~5h) ⚠️ **crítico** — ver [ADR-013](./docs/adr/ADR-013-meta-platform-2026.md) + [ADR-014](./docs/adr/ADR-014-criation-as-cdp.md). Após CDP entregar a captura, esta sessão é **fanout server-side**: lê de `tracking_events`, monta payload completo (IP+UA+fbp+fbc+external_id_hash+event_source_url+action_source+LDU), envia com mesmo `event_id` para Meta CAPI e Google EC. Schema deltas em `capi_events` (vira log auditorial); reescrever wizard "AEM priority order" (cap de 8 removido em jun/2025). **Pós:** 1.4.9.5 valida o conjunto 1.4.8 + 1.4.9 em prod real antes de 1.6 expor dado ao cliente.
- [ ] **1.4.9.5** — Shadow validation end-to-end em prod real (~8-10h trabalho ativo + 5-7d wait calendar) ⚠️ **gate arquitetural antes de 1.6 — não pule**. **Motivo:** subsistema de tracking (1.4.A captura + 1.4.B matcheia + 1.4.8 atribui + 1.4.9 envia) está **feature-complete** neste ponto. Até aqui validado contra fixtures sintéticas (219 testes verdes + smoke E2E manuais por sessão). Bugs reais (iOS Safari ITP, conflito com pixel/GTM pré-existente do cliente, encoding pt-BR em payload Hotmart prod, UTM `{{ad.name}}` literal em campanhas Meta com 5+ anos de naming caótico, EMQ baixo por hash format errado, dedup com pixel do cliente quebrado por event_id divergente, LDU flag não respeitando Consent state) só aparecem com dado real. **Última janela barata antes de 1.6/1.7 exporem o dado ao cliente** — matcher com falso positivo faz cliente ver venda atribuída à campanha errada no dashboard e perder confiança em 5min. Bug interno é barato; bug visível ao cliente é caro. Esta sessão valida **o conjunto 1.4.8 (UTM Stitcher) + 1.4.9 (CAPI fanout) em prod**, não em isolado. **Setup (sequencial):** (1) deploy staging com domínio próprio + HTTPS estável (Vercel `gru1`) servindo `criation-tracking.js` via CDN; (2) snippet instalado em 1 landing real (site da AGC ou cliente amigo alpha que sabe que é alpha); (3) 1 conexão Hotmart **prod** ativa recebendo webhooks reais (opcional Kiwify/Eduzz se disponíveis); (4) Meta ads ativos R$200-500 com convenção UTM correta apontando pra landing instrumentada; (5) consent banner mínimo (stub) pra Consent Mode v2 read funcionar; (6) **Meta Events Manager + Test Events** configurados pra ler EMQ shadow do dataset CAPI; (7) **Google Ads conversion action** com Enhanced Conversions habilitado em modo teste. **Camada 1 — captura (1.4.A):** script carrega sem conflito com pixel/GTM pré-existente, ITP Safari iOS não quebra cookie `_cio_vid`, Consent Mode v2 read funciona com banner real, eventos chegam no volume esperado (taxa ~ pageviews Meta − adblock), partição mensal `tracking_events` aguenta volume, rate limit 600/min não estoura em pico de campanha. **Camada 2 — matching (1.4.B):** visitor↔buyer dispara em venda real, reverse matching via `identify` funciona em fluxo lead magnet→compra, sem falsos positivos cross-workspace (RLS aguenta tráfego real), `matched_buyer_email_hash` retroativo populado nos `tracking_events` históricos do visitor. **Camada 3 — atribuição (1.4.8):** stitcher matcheia campanhas Meta reais (não fixture), TD-083 (`{{ad.name}}` literal) resolvido automaticamente via cascata visitor-aware, aggregates em `campaigns` incrementam atomic sem race, [PRE-PROD-VALIDATION-1.4.8.md](./docs/checklists/PRE-PROD-VALIDATION-1.4.8.md) executado com dado real. **Camada 4 — fanout (1.4.9):** **EMQ baseline medido no Events Manager (target ≥7)** após 7d de dado, event_id dedup com pixel do cliente confirmado (sem double-count em Events Manager → Diagnostics), hash format conforme spec Meta (SHA-256 lowercase trim em email/phone/external_id), `action_source` correto por evento type, LDU flag respeita Consent Mode state em tempo real, fbp/fbc presentes em ≥80% dos eventos client-side, Google EC Enhanced Conversions match rate ≥60% no conversions report, `capi_events` log preserva payload completo + response code Meta/Google pra auditoria. **Compliance:** TD-104 LGPD erasure path simulado com titular request real (visitor + buyer ambos apagados sem residual). **Entregáveis:** `docs/smoke/1.4.9.5-shadow-validation-e2e.md` documentando: cenários executados por camada, bugs encontrados (severidade P0/P1/P2), fixes aplicados nesta sessão, **EMQ baseline + Google EC match rate** (números reais, não suposição), novos TDs abertos. **Definição de pronto:** zero P0 nas 4 camadas; EMQ ≥7 no Events Manager (com 7d de dado); Google EC match rate ≥60%; GATE 1.4.8 + TD-104 verdes com dado prod; decisão escrita go/no-go pra 1.6 com lista do que mudou (ou não) na especificação do dashboard baseado no que o dado real revelou (ex: campos que aparecem mais vazios que o esperado, faixas de EMQ por evento type, etc).
