...
**Última atualização:** 2026-05-29 (sessão 1.9 analisar.video_ad)
**Fase ativa:** Fase 1 — Core Value
**Sync de criativos (follow-up 1.7) entregue 2026-05-29** (commit `4349d9f`, deploy Trigger `20260529.5`): `ad_creatives` agora popula com copy/thumbnail reais — `meta.service` expande `creative{...}` no edge de ads + `extractCreativeContent` (ladder object_story_spec/asset_feed_spec, spike-confirmado) + `upsertCreative` + wiring no campaign-sync. Validado real: 3613 criativos / 93% com copy. Destrava o picker da 1.9 e a CreativesGallery da 1.7. **TD novo:** full sync da conexão agência (32 ad accounts) leva >34min vs `maxDuration` 600s da task — backfill inicial de criativos pode precisar de múltiplos runs do cron (idempotente; não bloqueia).

**Próxima sessão:** **1.13 Checkout (Pix + cartão tokenizado) + paywalls + troca de plano** (~4h — consome billing.service.createSubscription do Asaas; UI transparente sem cara de Asaas) → 1.14 credit-expiration cron. **1.12 entregue** (billing backend + webhooks + allocate; pendente config Vercel/dashboards + E2E sandbox). **Fase 1 Estúdio Analisar fechada**: 1.5 + 1.6 + 1.7 + 1.7.5 + 1.8 + 1.9 + 1.10 + 1.11 + sync de criativos + refinamentos UX (cascata/nome/apagar/pastas) entregues. 1.4.9.5 (shadow validation) bloqueada por OAuth Google rework de vídeo (Trilha B). Suite **654/655 verde** (1 skip = integração créditos). **GATE pré-prod ainda aberto**: tracking install em 1 landing real (receita/ROAS zerados → toda análise diagnostica "landing"). Pendência herdada: Vinicius vai rotacionar a ANTHROPIC key (foi colada no chat) e revogar a antiga.
...
| Fase 1 — Core Value      | 🟡 Em andamento | 2026-05-07 | —          | 1.1-1.4.9.C + **1.5 (onboarding restruct)** + **1.6 (dashboard Mixpanel-style)** + **1.7 (/campanhas list+detail+comparativo)** + **1.7.5 (creditService)** + **1.8 (claude.service — wrapper + budget + judge)** fechadas (2026-05-28/29). 1.4.9.5 (shadow E2E) adiada por bloqueio externo (OAuth Google video rework). Próxima: 1.9 pipeline analisar.video_ad OU instalar tracking real |
...
- [x] **1.7.5** — `creditService` implementação completa (~5h) — ✅ entregue 2026-05-29 [v0.6 §4.10–§4.14]: `checkBalance` (pré-flight read-only, `safetyFactor` 1.5 pra análise), `consume` (atômico via **SELECT FOR UPDATE** `.for('update')` — relational API não suporta lock; idempotência + fix de race unique-violation 23505), `refund` (reverte ao balde de origem lido de `metadata.deduction`), `expireBatch(asOf, opts?)` (baldes + packs granular via `pack_purchases`, 1 tx/workspace, `onConflictDoNothing` por dia — task 1.14 só liga o cron), `getHistory` (keyset). Função pura `computeDeduction` (ordem de consumo §4.12 = `expires_at` ASC, não FIFO) + `sumEligible`. Erro de fluxo = shape discriminado `ok:false` (Regra 7), não throw. 6 commits, +34 testes unit + **teste de integração race condition validado contra Supabase dev real** (10 consumes paralelos sobre saldo 5 → 5 ok / 5 recusados / saldo final 0, nunca negativo; `FOR UPDATE` provado). Smoke `docs/smoke/1.7.5-credit-service.md`. Pack-expiry-no-balde tratado como far-future (limitação documentada). TD-124 (retrofit idempotency em `allocate`) + TD-125 (`revoke`→`refund` em process-gateway-event) abertos.

#### Semanas 2-3 — Estúdio Quick

- [x] **1.8** — claude.service + bloco de transição + integração com creditService (~4h) — ✅ entregue 2026-05-29 [v0.6 §1.8]: `@anthropic-ai/sdk` + `lib/claude/models.ts` (Quick=Sonnet 4.6, Deep=Opus 4.8, Trivial/judge=Haiku 4.5 — IDs atuais, pin antigo aposentado no CLAUDE.md) + pricing/estimateCostUsd. `lib/claude/budget.ts` hard cap Regra 20 (Starter R$10/Pro R$40/Agency R$120, soma claude_request_logs.cost_usd, USD_TO_BRL). `lib/validators/bloco-transicao.ts` (input padronizado) + `lib/claude/validators/` (output). `claude.service.analyze` com prompt caching (cache_control ephemeral), retry 429/5xx (sem 4xx), validateOutput Zod, observabilidade claude_request_logs, **dois gates** (budget hard-cap SEMPRE + créditos checkBalance 1.5x pré-flight/consume on-success §4.11 só com ctx.credits) + structural-judge (Haiku, refaz 1x) + generateCopy. Erro = shape discriminado (Regra 7; neverthrow descartado). prompts v1.0.0 + seed prompt_versions status='production' (validado dev: 2 rows). 5 commits, +40 testes (mock SDK). Smoke `docs/smoke/1.8-claude-service.md`. streamAnalyze→2.9.
- [x] **1.9** — `analisar.video_ad` (Quick) + fluxo /estudio/analisar/nova com preview de custo — ✅ entregue 2026-05-29 [v0.6 §1.8.2]: **primeiro pipeline real fim-a-fim**. Fluxo: `/estudio/analisar/nova` (Server Component: lista campanhas ACTIVE + saldo + custo) → `<NovaAnaliseForm>` (tabs tipo de ativo só video_ad ativo, picker campanha→criativo via action `getCreativesForPicker`, depth Quick, contexto opcional, **preview "1 crédito · ~30s · saldo"**) → action `createAnalysis` (thin: Zod + resolveContext + **checkBalance preflight 1.5x** + cria row `queued` + dispara task + grava trigger_job_id) → task Trigger.dev `estudio-analisar-video-ad` (status running → `analysis.service.runVideoAdAnalysis`) → page `[id]` (3 estados running/completed/failed, render MVP do schema Quick). **analysis.service**: `detectBottleneckHint` (heurística pura CTR/CVR/ROAS → stage creative/landing/offer/overall), `buildBlocoTransicao` (monta `blocoTransicaoSchema` de `getCampaignHeader`+`getCampaignKpis`+`getCreativeForAnalysis`+`profile_context`; money cents→reais; **hookRate/holdRate=null** Meta v25), `runVideoAdAnalysis` (valida bloco antes do Claude → `analyze(analysisQuickPrompt, bloco, {credits:{cost:1, idempotencyKey:analysisId}}, {judge:true})` → persiste `analysis_results` + completed + propaga credit_transaction_id via `getTransactionByIdempotencyKey`). Queries novas: `pipeline-costs.ts` (`getPipelineCost`), `analyses.ts` (create/get+result/list/updateStatus/insertResult), `getCreativeForAnalysis`/`getTransactionByIdempotencyKey`. Helper `resolveCurrentWorkspaceId` (auth/workspace.ts). **Gotcha #1**: `ANTHROPIC_API_KEY` adicionada ao `TRIGGER_RUNTIME_ENV_KEYS` (trigger.config.ts) — **pendente `pnpm trigger:deploy`** pra task rodar no cloud. **Escopo MVP**: só origem campanha conectada (upload/externo = em breve); análise dirigida por métricas+copy (sem Deepgram/frames). Erro = shape discriminado (Regra 7). +17 testes (heurística+bloco+orquestração+action happy/erros). Suite **632/633** (1 skip). tsc+lint verde. Smoke `docs/smoke/1.9-analisar-video-ad.md`. **Pós**: 1.10 polling automático (substitui refresh manual), 1.11 relatório rico 8 seções.
- [x] **1.10** — Typewriter effect e polling otimizado — ✅ entregue 2026-05-29 (commit `a6c1d43`): a página `/estudio/analisar/[id]` atualiza sozinha quando a análise termina (fim do refresh manual da 1.9, reportado em teste no preview). **QueryProvider** (TanStack Query) wired no root layout — 1ª adoção real do react-query (estava no package.json, sem provider). Server action `getAnalysisStatus(id)` (workspace-scoped, read-only). Client `RunningWatcher` (useQuery `refetchInterval` 2.5s enquanto running → para no estado terminal → `router.refresh()` re-renderiza o Server Component com o resultado). `Typewriter` revela o summary progressivamente (respeita prefers-reduced-motion; reset via `key={summary}`). +3 testes `getAnalysisStatus`. Suite 643/644. tsc+lint verde. Push develop → preview Vercel.
- [x] **1.11** — Página de resultado seccionada + footer com saldo — ✅ entregue 2026-05-29 (commit `98b157e`): relatório profissional em seções na `/estudio/analisar/[id]` (estado completed): header executivo (veredito/score/gargalo·severidade + ações Nova análise/Exportar JSON reais; Comparar/Modelar/Rodar de novo "em breve") · resumo estratégico (typewriter) · diagnóstico do gargalo (borda por severidade) · pontos fortes/fracos · sugestões priorizadas (cards P1/P2/P3 por ordem) · **dados do funil** (do snapshot do BLOCO) · **footer "N crédito(s) consumido(s) · saldo atual: N"**. Schema: `analysis_results.input_snapshot jsonb` (DDL aditivo via mcp) persistindo o BLOCO imutável que a IA viu — `runVideoAdAnalysis` salva; análises antigas mostram fallback. `ExportJsonButton` client. Suite 654/655, tsc+lint verde. **Adaptação do escopo:** a v0.6 §1.8.2 lista 8 seções do relatório **Deep**; o pipeline Quick não gera evidências visuais/copy pronto/feedback/tracking — esses ficam pra Fase 2 (2.9+), marcados "em breve".

#### Semana 3 — Billing

- [x] **1.12** — billing.service abstrato (Asaas + Stripe) + allocate em webhooks — ✅ entregue 2026-05-29 (commits 564df59→f37a625, deploy Trigger `20260529.6`): billing **merchant** (Criation vende). `billing/types.ts` (BillingEvent normalizado + BillingAdapter) · `asaas.adapter` **completo** (REST `access_token`, createCustomer/createSubscription transparente Pix+cartão tokenizado/cancel/changePlan/refund, validateWebhook `asaas-access-token`, parseEvent) · `stripe.adapter` **enxuto** (constructEvent + parseEvent reais; create/cancel = NOT_IMPLEMENTED p/ 1.13) · `billing.service` (roteamento país BR→asaas/lock ADR-007 + handleWebhook) · rotas `/api/webhooks/asaas|stripe` (runtime nodejs, validate→dedup `recordProcessedWebhook`→enfileira→200) · task `process-billing-event`→`applyBillingEvent` (invoice_paid→upsertSubscription+setWorkspacePlan+allocate 'subscription'; pack→allocate 'pack'; refund→creditService.refund por idempotencyKey; cancel→marca sem revogar). **Idempotência 2 camadas** (processed_webhook_events + idempotencyKey=id do evento; allocate/refund já idempotentes). +31 testes. Suite 675/676, build+deploy ok. **PENDENTE config/E2E:** setar `ASAAS_API_KEY`/`ASAAS_WEBHOOK_SECRET`/`ASAAS_BASE_URL`/`STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET` na Vercel (Prod+Preview) + registrar URLs de webhook nos dashboards Asaas/Stripe + E2E sandbox (criar sub→webhook→crédito). **Checkout (loja Pix/cartão) = 1.13.**
