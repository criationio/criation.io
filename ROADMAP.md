...
**Próxima sessão:** **1.9 estudio.analisar.video_ad** (~5h — primeiro pipeline real + UI /estudio/analisar/nova, chama `analyze(analysisQuickPrompt, bloco, {...ctx, credits})`) OU **Tracking install em 1 landing real** (~30min). **1.5 + 1.6 + 1.7 + 1.7.5 + 1.8 entregues**. 1.4.9.5 (shadow validation) continua bloqueada por OAuth Google rework de vídeo (Trilha B). Suite **615/616 verde** (1 skip = integração créditos). ANTHROPIC_API_KEY precisa estar no Vercel/env antes de 1.9 chamar a API real.
