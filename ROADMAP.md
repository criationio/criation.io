...
**Última atualização:** 2026-05-29 (sessão longa — 1.9→1.12 + rename planos; ver `docs/HANDOFF-2026-05-29.md`)
**Fase ativa:** Fase 1 — Core Value

**Próxima sessão:** **terminar E2E sandbox 1.12** (Vinicius cola chaves Asaas sandbox no `.env.local` → Claude sobe túnel + registra webhook → Vinicius cria sub teste → validar allocate idempotente; detalhes no HANDOFF) → **1.13 Checkout transparente (Pix + cartão tokenizado) + paywalls + troca de plano** (consome `billing.service.createSubscription` do Asaas, já pronto) → 1.14 credit-expiration cron.

**Entregue hoje (2026-05-29):** 1.9 (`analisar.video_ad`) + 1.10 (polling+typewriter) + 1.11 (resultado seccionado + footer saldo + export PDF/DOCX/JSON) + **sync de criativos** (gap 1.7; 3613 criativos / 93% copy) + **refinamentos UX** Estúdio (cascata conta→campanha→anúncio + nome exato Meta, nomear/apagar análise, pastas) + 2 fixes prod (loop dashboard↔bem-vindo, cor Dialog/Input) + **renomeação planos** Pro/Advanced/Enterprise (catálogo único) + **1.12 billing backend** (Asaas full + Stripe lean + webhooks + `process-billing-event`→allocate). Suite **675/676 verde**. Trigger prod `20260529.6` (20 tasks). Tudo em `develop`; prod/main ainda sem nada de hoje.

**Pendências:** 1.12 config Vercel/dashboards + E2E sandbox (próxima ação). **GATE pré-prod aberto**: tracking install em 1 landing real (receita/ROAS zerados → toda análise diagnostica "landing"). **TD:** sync agência (32 contas) >34min vs maxDuration 600s. 1.4.9.5 bloqueada por OAuth Google (rework vídeo). Vinicius rotaciona a ANTHROPIC key (colada no chat).
