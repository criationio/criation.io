...
**Bloqueios pré-cliente real:** GATE pré-prod [1.4.8 checklist](./docs/checklists/PRE-PROD-VALIDATION-1.4.8.md) (UTM Stitcher) **+** [TD-104](./docs/tech-debt.md) LGPD erasure path **+** [TD-108](./docs/tech-debt.md) retention 30d pra plain IP/UA (audit 1.4.9) **absorvidos pela sessão 1.4.9.5** — executados com dado real de prod sobre o conjunto 1.4.8 + 1.4.9, não em isolado. Outros TDs condicionais: TD-109 (gateway-only fanout) quando cliente sem script conectar, TD-111 (CTWA payload validation) quando cliente com WhatsApp ads ativar.

**Ação operacional pendente (1.4.9):** `pnpm trigger deploy` precisa rodar pra ativar `fanoutMetaCapiTask` + `fanoutMetaCapiPickupCron` + versão atualizada do `processTrackingEventTask`. Sem deploy, fluxo CAPI fica dormente em prod.
